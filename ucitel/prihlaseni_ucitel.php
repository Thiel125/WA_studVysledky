...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
  ...
  $sql = "SELECT ID_ucitel FROM ucitel WHERE email = '$email' and heslo = '$password'";
  $result = mysqli_query($conn, $sql);
  $pocet = mysqli_num_rows($result);

  if ($pocet == 1) {
    $zaznam = mysqli_fetch_assoc($result);
    $_SESSION['prihlaseni_ID_ucitel'] = $zaznam['ID_ucitel'];
    header("location: hlstranka_ucitel.php?id=".$zaznam['ID_ucitel']);
  } else {
